...
int main()
{
	...
	XGpio input; //구조체 XGpio를 input이름으로 할당
	int switch_data = 0; //스위치 상태값 체크용 변수
	int checkIDLE = 0; //노래정지상태 체크용 플래그비트
	XGpio_Initialize(&input, XPAR_AXI_GPIO_2_DEVICE_ID); //input을 GPIO2로부터 값입력 시작
	XGpio_SetDataDirection(&input,1,0xF); //데이터 입출력방향 입력으로 설정
    ...
    for(;;) {
		...
		int currentFile = 0;
		for(;;) {
			switch_data = XGpio_DiscreteRead(&input, 1); //딥스위치 값을 switch_data에 1채널로 값을 넣음
			printf("\033[2J\033[H\r\n");
			...
			for (int cnt=0;!isKeyPressed();++cnt) {
				if ( cnt % 100000 == 0 && lastValue!=thePlayBuffer.percentFinishedUpdated) {
					...
					if (adau1761_dmaBusy(&codec)) {
						...
						printf(" Now Play %s", files[currentFile]); //현재플레이중인 노래 확인용
						checkIDLE = 1; //노래 재생중 플레그비트 셋
					}
					else {
						switch_data = XGpio_DiscreteRead(&input, 1); //노래가 종료되면 현재 딥스위치 값 확인
						if(checkIDLE != 0){//노래가 플레이 중이였으면
						switch(switch_data){
							case 0b1://"one music rotate\r\n");
								playWavFile(files[currentFile]); //현재 커서가 있는 노래 반복
								checkIDLE = 1;
								break;
							case 0b10://"whole music rotate\r\n");
								if(currentFile+1<filesNum)
									++currentFile; //현재 커서 하나 +1
								else if(currentFile+1==filesNum) currentFile = 0; //만약 노래리스트 마지막이면 처음으로
								playWavFile(files[currentFile]); //노래재생
								checkIDLE = 1;
								break;
							case 0b100://"whole Random rotate\r\n");
								currentFile = (rand()%(filesNum)); //stdlib.h의 rand함수로 나머지값 추출, 나눠야 0~마지막곡까지 나머지값 추출
								playWavFile(files[currentFile]);
								checkIDLE = 1;
								break;
							default:
								printf("   IDLE   "); //디폴트 상태일경우 노래 재생 종료
								checkIDLE = 0;
								break;
						}
						}
